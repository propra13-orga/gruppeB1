...
/*
 * Scene_Map.java
 * Diese Scene ist das Herzst�ck des Programms, da sie den gesamten Spielablauf regelt. Sie
 ...
 * Kurz gesagt �berpr�ft updateLogic alle Tasteneingaben und verarbeitet sie entsprechend
 * (Spielerbewegung, Men�aufruf, etc.) und zeichnet anschlie�end die Karte samt Sprites neu.
 * 
 * F�r eine detailierte Beschreibung siehe updateLogic und updateScreen
 */
...
public class Scene_Map extends Scene {
	...
	private void updateLogic() {
		//�berpr�ft, ob eine Bewegungstaste gedr�ckt wurde und
		//bearbeitet die Koordinaten des Characters entsprechend
		//Initialisiert (wenn n�tig) auch eine Bewegungsanimation
		
		//checkMenu gibt true zur�ck, falls tats�chlich die Scene gewechselt wurde. In
		//diesem Fall soll die update Methode nat�rlich so schnell wie m�glich abbrechen
		if (check_menu()) return;
		
		//Falls, der Spieler gerade steuerbar ist (also nicht schon in einer
		//Bewegungsphase), dann pr�fe jetzt, ob er bewegt wurde
		if (!skip_moving) {
		
			//Alte Koordinaten des characters speichern um zu
			//pr�fen, ob eine Bewegung animiert werden muss
	...
	private void check_walking() {
		//Falls eine Pfeiltaste gedr�ckt wurde, wird versucht den Spieler
		//zu bewegen (das ist nur m�glich, wenn er versucht, auf ein begehbares
		//Feld zu gelangen
	...
	private void player_animation() {
		//Der movecounter von jedem Sprite ist so gro�, wie
		//ein Tile (momentan 32). W�hrend einer Bewegung wird er hochgez�hlt
		//, um den character Pixelweise zu bewegen und zu animieren
		//Wenn der movecounter 32 (die Tilegr��e) erreicht hat
		//dann ist die Animation vorbei und alles wird zur�ckgesetzt.
		...
		if (player.movecounter >= Map.TILESIZE) {
			...
			player.save_position();
			//Schrittanimation f�r die n�chste Bewegung wird gewechselt
	...
	private void drawSprites() {
		...
		// !!! WICHTIG !!!
		//Momentan werden Sprites noch nicht danach sortiert, wer "�ber" wem steht
		...
		for (Sprite s : sprites) {
			...
			//Berechne tats�chliche Pixelkoordinaten in Abh�ngigkeit von
			//Animation und Scrolling
			int new_x=-100;
			int new_y=-100;
			//Falls scrolling aktiviert ist, ver�ndert der Character zwar seine Mapkoordinaten
